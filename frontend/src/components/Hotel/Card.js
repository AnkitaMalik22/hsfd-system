...
export default function CardFood({food}) {
  ...
  {/* </Link> */}
        ...
        >
    {   !expanded ?  <IconButton aria-label="comment"><Comment /> {
            food.requests.length >0 ? <Typography paragraph> {food.numOfRequests}</Typography> : <Typography>0</Typography>
...
<Paper
    ...
    maxWidth: 360,
      ...
      <div>
        ...
        <div>
        {
foodDesc.requests.length<=0 ? <Chip  color="primary" label="No Requests Yet!" /> :<Chip  color="primary" label={`Requested by - ${foodDesc.requests.length} volunteers ` } />  
  }
     ...
     <React.Fragment>
      {/* key={request.id} */}
       <ListItem alignItems="flex-start" >
        ...
        />
         ...
         onClick={()=>{request.status ? alert("alreay accepted")  : handleAccept(foodDesc.id,request.user)}}
          />
      </ListItem>
      ...
      <div >
        <Button color="primary" onClick={()=>{handleDelete(foodDesc.id)}}>
          ...
          Delete Food</Button>
       {
         foodDesc.numOfRequests >0 &&  <Chip onClick={()=>foodDesc.picked ? alert("already picked!") : handlePicked()} 
         disabled= { foodDesc.picked  ? true : false  }
    
        variant="contained" label={foodDesc.picked ? "Picked" : "Mark as Picked"} />
       }
